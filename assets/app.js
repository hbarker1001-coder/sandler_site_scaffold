...
async function renderIndex(){
  ...
  modules.forEach(m => {
    const moduleClips = clips.filter(c => c.module_id === m.module_id)
      .sort((a,b) => Number(a.order||999) - Number(b.order||999));
    const total = moduleClips.length;
    const st = getState(m.module_id);
    const completed = st.result?.pass;
    const lastClipId = st.lastClipId || (moduleClips[0] && moduleClips[0].clip_id);
    const primaryLabel = completed ? 'Review module' : (lastClipId ? 'Resume module' : 'Start module');
...
async function renderClip(){
  ...
  by('#clipTitle').textContent = clip.clip_title || clip.clip_id;
  ...
  if(!clip.audio_url){
    ...
    if(audioNotice){
      audioNotice.textContent = 'No audio file configured for this clip yet.';
  ...
  form.innerHTML = '';
  const s0 = getState(module_id);

  qs.forEach((q,i) => {
    const id = `r_${clip.clip_id}_${q.question_order}`;
    const div = document.createElement('div');
    div.className = 'q';
    div.innerHTML = `
      <label for="${id}">${i+1}. ${q.question_text}</label>
      <textarea id="${id}" placeholder="Type your reflection here..."></textarea>
    `;
    form.appendChild(div);
    if(s0.reflections && s0.reflections[id]){
      div.querySelector('textarea').value = s0.reflections[id];
    }
  });

  by('#saveReflections').addEventListener('click', () => {
    const s = getState(module_id);
    ...
    qs.forEach(q => {
      ...
      const el = by('#'+id);
      if(el){
        s.reflections[id] = el.value.trim();
      }
    });
    setState(module_id, s);
...
async function renderQuiz(){
  ...
  by('#submitQuiz').addEventListener('click', () => {
    const st = getState(module_id) || {};
    let correct = 0;
  ...
  by('#certBtn').addEventListener('click', () => {
    const user = getUser();
    const st = getState(module_id) || {};
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if(page === 'index') renderIndex().catch(err => console.error(err));
  if(page === 'clip') renderClip().catch(err => console.error(err));
  if(page === 'quiz') renderQuiz().catch(err => console.error(err));
});
